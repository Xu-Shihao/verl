...
from transformers import AutoConfig, AutoModelForCausalLM, PreTrainedModel, StoppingCriteria, StoppingCriteriaList
...
class SFTTrainer2(FSDPSFTTrainer):
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        self.curr_eos = [self.tokenizer.eos_token_id, self.tokenizer.pad_token_id]
        
        target_sequences = ["</search>", " </search>", "</search>\n", " </search>\n", "</search>\n\n", " </search>\n\n"]
        self.stopping_criteria = StoppingCriteriaList([StopOnSequence(target_sequences, self.tokenizer)])
    ...
    def _build_dataloader(self):
        ...
        if rank == 0:
            print(f'loading validation data from {config.data.val_files}')
    ...
    def _build_model_optimizer(self):
        ...
        target_sequences = ["</search>", " </search>", "</search>\n", " </search>\n", "</search>\n\n", " </search>\n\n"]
        self.topping_criteria = StoppingCriteriaList([StopOnSequence(target_sequences, self.tokenizer)])
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(self.train_dataloader,
                ...
                global_step += 1
                if rank == 0:
                    before_inputs = data['input_ids']
                    print(f'before, input_ids: {before_inputs}, shape: {before_inputs.shape}')
                    import numpy as np
                    np.save(open('input.npy', 'wb'), data['input_ids'].numpy())
                    np.save(open('mask.npy', 'wb'), data['attention_mask'].numpy())
                
                data = TensorDict(data, batch_size=self.config.data.train_batch_size).cuda()
                
                if rank == 0:
                    after_inputs = data['input_ids']
                    print(f'after, input_ids: {after_inputs}, shape: {after_inputs.shape}')
    ...
    def _generate_sequences(self, input_ids, attention_mask, do_sample=True, temperature=0.7, use_cache=False, max_turn=2):

        # Encode the chat-formatted prompt and move it to the correct device
        cur_turn = 0
        rolling_ids = input_ids
        #print(f'rolling_ids.shape: {rolling_ids.shape}')
        #print(f'rolling_ids: {rolling_ids}')
        rolling_attention_mask = attention_mask
        #print(f'rolling_mask.shape: {rolling_attention_mask.shape}')
        #print(self.config.data)
        #print(self.fsdp_model.module)
        with torch.distributed.fsdp.FullyShardedDataParallel.summon_full_params(self.fsdp_model):
            while True:
                # Generate text with the stopping criteria
                rolling_ids = self.fsdp_model.module.generate(
                    rolling_ids,
                    attention_mask=rolling_attention_mask,
                ...
                if rolling_ids[0][-1].item() in self.curr_eos or cur_turn > max_turn:
                    generated_tokens = rolling_ids[0][input_ids.shape[1]:]
                    output_text = self.tokenizer.decode(generated_tokens, skip_special_tokens=True)
                    #print(output_text)
                    return {"logits": logits, "generated_text": output_text}

                generated_tokens = rolling_ids[0][input_ids.shape[1]:]
                output_text = self.tokenizer.decode(generated_tokens, skip_special_tokens=True)
                
                tmp_query = get_query(self.tokenizer.decode(rolling_ids[0], skip_special_tokens=True))
                if tmp_query:
                ...
                info_res = self.curr_template.format(search_results=search_results)
                info_ids = self.tokenizer.encode(info_res, return_tensors='pt').to(rolling_ids.device)
                rolling_ids = torch.cat((rolling_ids, info_ids), dim=1)
                rolling_attention_mask = torch.ones_like(rolling_ids)
            #print(search_text)
        ...
        with context:
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                if not use_sp:
                    ...
                    output = self._generate_sequences(input_ids=input_ids, 
                    ...
                    # Forward pass
                    output = self._generate_sequences(input_ids=input_ids_rmpad_sliced, 
                                                      attention_mask=None, 
                                                      use_cache=False)
...
class TaskRunner:
    ...
    def run(config):
        ...
        trainer = SFTTrainer2(config=config, device_mesh=device_mesh, ulysses_device_mesh=ulysses_device_mesh)
