...
MODEL_BASE_NAME="qwen3-8B_interactive_empathetic_length_reward"
NNODES=1
...
# 项目配置
project_name='SMHC_Interactive_Diagnosis_Empathetic_LengthReward'
...
max_response_length=3072  # 增加以支持多轮对话
...
# 长度奖励参数（梯形奖励函数）- 鼓励充分问诊
USE_LENGTH_REWARD=True  # 启用长度奖励
LENGTH_REWARD_WEIGHT=0.2  # 长度奖励权重（建议0.1-0.3）
LENGTH_MIN_TURNS=10  # 最小合理轮数
LENGTH_OPTIMAL_START=15  # 最优区间开始
LENGTH_OPTIMAL_END=25  # 最优区间结束（满分在15-25轮）
LENGTH_MAX_TURNS=50  # 最大合理轮数

# ICD奖励参数
USE_ICD_REWARD=True

# Tool 配置文件路径（使用带长度奖励的配置）
TOOL_CONFIG="${HOME}/psy_r1/consultation_grpo_trainer/patient_tool_config_with_length_reward.yaml"
...
# 启动 GRPO 交互式问诊训练（共情式 + 长度奖励模式）
echo "========================================="
echo "GRPO 交互式问诊训练 (共情式 + 长度奖励)"
echo "========================================="
echo "模型: ${MODEL_PATH}"
...
HYDRA_FULL_ERROR=1 && python3 -m psy_r1.verl.trainer.main_ppo_psy \
    ...
    +reward_model.use_length_reward=${USE_LENGTH_REWARD} \
    +reward_model.length_reward_weight=${LENGTH_REWARD_WEIGHT} \
    +reward_model.length_min_turns=${LENGTH_MIN_TURNS} \
    +reward_model.length_optimal_start=${LENGTH_OPTIMAL_START} \
    +reward_model.length_optimal_end=${LENGTH_OPTIMAL_END} \
    +reward_model.length_max_turns=${LENGTH_MAX_TURNS} \
