...
基于 main_ppo_psy.py，使用 v8 混合奖励函数
"""
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, is_train=True):
    ...
    if "custom_cls" in data_config and data_config.custom_cls.get("path", None) is not None:
        ...
        if not issubclass(dataset_cls, Dataset):
            raise TypeError(f"Custom dataset class must inherit from Dataset")
...
def create_rl_sampler(data_config, dataset):
    ...
    if sampler_config is not None and sampler_config.get("class_path", None) is not None:
        curriculum_class = load_extern_type(sampler_config.class_path, sampler_config.class_name)
